...
The valid responses should be between zero and 100 percent of the time, so Leslie decides to recode values outside this range to be `NA`. She goes to the GSS Data Explorer [@GSS] and looks up the variable to figure out how it was coded. Leslie examines the codebook and it shows that the two values outside the range are -1 and 999 (Figure \@ref(gss.usetech))
...
The p-value in this case is `r format(techuse.by.deg$p.value, scientific = FALSE)`. The value of an $F$-statistic being this large or larger happens a tiny percent of the time when the null hypothesis is true. 
...
```{r}
# Tukey post-hoc test for tech.by.deg
...
Leslie wants to incorporate the means of the combined groups into the final interpretation with the planned comparisons. She turns to Nancy to write some code:
...
gss.2018.cleaned %>% 
  mutate(DEGREE = factor(DEGREE, labels = c(NA,
                      NA, "jr college",
                      "more than jr college", "more than jr college"))) %>% 
  group_by(DEGREE) %>%
  summarise(m.techuse = mean(x = USETECH, na.rm = T),
            sd.techuse = sd(x = USETECH, na.rm = T))
...
After learning about Cramer's V for $\chi^2$ (see Section \@ref(cramersv)) and Cohen's D for $t$-tests (see Section \@ref(cohensd)), Leslie asks Kiara what the effect sizes are to report for ANOVA. Kiara explains that eta-squared, $\eta^2$, is the proportion of variability in the outcome variable that is explained by the statistical model and this is the commonly used effect size for ANOVA [@etasq]. Nancy is not so sure! She recently read a paper by Susan Skidmore and Bruce Thompson [@skidmore2013bias] that recommends $\omega^2$ instead. The $\eta^2$ has a known positive bias and been used widely mainly because it is the effect size that is available in some statistical software programs [@skidmore2013bias]. The $\omega^2$ effect size has the same general meaning, is adjusted to account for the positive bias, and is more stable when assumptions are not completely met. Kiara writes out the formula for $\eta^2$ to help her explain how it works:
...
Where $n$ is the number of observations and $k$ is the number of groups. 

Nancy jumps in with some R code to compute the $\omega^2$ for the technology use analyses. First she explains that the two functions they have used so far, `oneway.test()` and `aov()`, do not compute $\omega^2$ as part of the output. However, there are R packages that do compute it. Even so, Nancy recommends sticking with `aov()` and using the output from `aov()` to compute $\omega^2$. She shows the team how by examining the output from the earlier `aov()`:
...
Everything they need to compute $\omega^2$ is in the output:
...
Kiara is a little uneasy with Nancy's solution here and asks, for the sake of reproducibility, if there is any way to do this calculation directly in R without retyping the numbers. Nancy says there is, but it requires some fancy coding to access the values in the output. Kiara thinks that sounds better than typing out the values, so she and Nancy work on a set of instructions for Leslie to use if she is so inclined (see Box \@ref(ch7kiara)).

The final thing Leslie wants to know is the cutoffs for small, medium, and large effects. Are they the same as for the other effect sizes? Kiara says unfortunately they are not, here are the cutoffs for $\eta^$ effect size:

* $\omega^2$ = .01 to $\omega^2$ < .06 is a small effect 
* $\omega^2$ = .06 to $\omega^2$ < .14 is a medium effect 
* $\omega^2 \ge$ .14 is a large effect 

In this case, the effect size is medium. There is a medium strength relationship between degree and time using technology. Leslie copies her interpretation from above and adds effect size:
...
The first assumption for ANOVA is that the data are normally distributed. There are several ways to assess normality. Visually, a histogram, density plot, or QQ-plot is useful for identifying normal and non-normal data distribution. Statistically, a Shapiro-Wilk test can be used. 

### Testing normality 

Leslie decides to start with density plots and uses the same colors they used for the boxplots at the beginning. 
...
None of the groups look normally distributed. While Leslie was making density plots, Nancy put together some qq-plots to confirm:

```{r echo = FALSE, fig.cap="Distribution of technology use by degree."}
#graph tech use by degree
gss.2018.cleaned %>%
  drop_na(USETECH) %>%
  ggplot(aes(sample = USETECH, color = DEGREE)) + 
  stat_qq() + 
  facet_wrap(~ DEGREE, nrow = 2) + 
  geom_abline(intercept = mean(gss.2018.cleaned$USETECH, na.rm = TRUE), 
              slope = sd(gss.2018.cleaned$USETECH, na.rm = TRUE)) +
  scale_color_manual(values = c("#78A678", "dodgerblue2","#7463AC", 
                               "deeppink", "gray80"), guide = FALSE) +
  theme_minimal() + 
...
Leslie notes that none of the groups appear to be normally distributed based on either qq-plot. She also notices that the floor and ceiling values appear to be driving some of the non-normality. The Shapiro-Wilk test does not seem necessary in this case given the big deviations from normal in the histograms and qq-plots, however, Nancy suggests that they try it just to confirm. Leslie reminds herself that the Shapiro-Wilk test is testing the null hypothesis that the data are normally distributed. Nancy remembers that she had trouble before with using this test for multiple groups. She tries something new with the R code, which is to use the usual `summarize()` after `group_by()` but just print the p-value for the tests:
...
gss.2018.cleaned %>%
  drop_na(USETECH) %>%
  group_by(DEGREE) %>%
  summarize(shap.pval = shapiro.test(x = USETECH)$p.value)
```

Based on the p-values, all five of the Shapiro-Wilk tests were statistically significant, indicating that the null hypothesis for this test (i.e., the data are normally distributed) was rejected in each group. The team feels they have plenty of evidence that the data are not normally distributed. They move on to the next assumption.
...
Kiara explains to Leslie that the two main assumptions for ANOVA, normality and homogeneity of variances, have different alternatives when each assumption is not met. 

### Calculating an alternate $F$-statistic for failing the homogeneity assumption

The first option Kiara brings up is for when the normality assumption is met but the homogeneity of variances assumption fails, the standard approach is to use ANOVA but compute an alternate F statistic that does not rely on equal variances. There are two alternate $F$-statistics that are widely used for this purpose:
...
The $F$-statistic is then computed using the same $F$ formula but with the means computed for the transformation ($t$) of the variable rather than the raw value ($y$) of the continuous variable:
...
While there are R packages that can be used to compute the Brown-Forsythe directly, another option is to transform the outcome variable and use the `aov()` command used for ANOVA. Nancy shows the team how to create this new variable using `abs()` to get the absolute value of the difference between each value of USETECH and the median of USETECH, making sure to remove the NA in the `median()` function so it works.
...
gss.2018.cleaned <- gss.2018 %>% 
  ...
  mutate(HAPPY = factor(x = HAPPY, labels = c("very happy", 
                                              "pretty happy",
                                              "not too happy"))) %>%
  mutate(usetech.t = abs(USETECH - median(USETECH, na.rm = TRUE)))

# check new variable
summary(object = gss.2018.cleaned$usetech.t)



```

The new variable looks good, Leslie copies the `oneway.test()` from earlier, changes it to use usetech.t instead, and starts the NHST process.
...
The p-value in this case is `r format(usetech.t.by.degree$p.value, scientific = FALSE)`. The value of an $F_{BF}$-statistic being this large or larger happens a tiny percent of the time when the null hypothesis is true. 

#### NHST Steps 4 & 5: Interpret the probability and write a conclusion 

Leslie writes her conclusion:

> The results show a statistically significant difference in the mean of the transformed usetech variable by degree group [F(`r usetech.t.by.degree$parameter[1]`, `r usetech.t.by.degree$parameter[2]`) = `r round(usetech.t.by.degree$statistic, 2)`; p < .05]. 

Leslie is interested in how the transformed data used in these calculations looks; she decides to compute the descriptive statistics and examine a graph of the transformed variable to better understand the results.
...
```{r}
# graph transformed USETCH variable
gss.2018.cleaned %>%
  drop_na(usetech.t) %>%
  ggplot(aes(y = usetech.t, x = DEGREE, fill = DEGREE, color = DEGREE)) + 
  geom_boxplot(alpha = .8) + 
  geom_jitter(alpha = .3) +
  scale_fill_manual(values = c("#78A678", "dodgerblue2","#7463AC", 
                               "deeppink", "gray80"), guide = FALSE) +
...
Leslie notices that the transformation has reduced the differences between the junior college, bachelor, and graduate categories, with the boxplot showing nearly the same median value for the transformed variable. Kiara reminds Leslie that they have another option for this situation. The Welch's variation on ANOVA is also useful when the assumption of homogeneity of variances is not met. 

While Leslie has been working on the Brown-Forsyth test, Kiara looked up some additional information on the Welch test. Rather than use a transformed outcome variable, the main idea behind the Welch $F$-test is to use weights in the calculation of the group means and overall mean (also known as the **grand mean**). The weight is computed for each group to account for the different variances across groups:
...
$$
\begin{equation}
\overline{y}_{welch} = \frac{\sum_{j=1}^k{w_k \overline{y}_{k}}}{\sum_{j=1}^k{w_k}}
 (\#eq:weightwelch2)
\end{equation}
$$

Where $w_k$ is the weight for group $k$, $\overline{y}_k$ is the mean of the continuous variable for group $k$. The $\sum_{j=1}^k$ is the sum of each group from the first group, $j=1$ to the last group, $j=k$. The Welch grand mean is then used in the final computation of the Welch $F$-statistic, which gets a little complex [@field2013discovering]:

$$
\begin{equation}
F_{welch}=\frac{\frac{\sum_{j=1}^k{w_k(\overline{y}_k-\overline{y}_{welch})^2}}{r-1}}{1+\frac{2}{3}\Bigg(\frac{\frac{3\sum_{j=1}^k\bigg(1-\frac{w_k}{\sum_{j=1}^k{w_k}}\bigg)^2}{n_k-1}}{k^2-1}\Bigg)(k-2)}
 (\#eq:cov)
\end{equation}
$$

Leslie takes one look at the formula and asks if there is an R command to compute the value. Nancy explains that the `oneway.test()` command computes Welch's F when the option `var.equal =` is set to be false. Leslie gets her NHST process started:

#### NHST Step 1: Write the null and alternate hypotheses

The null and alternate hypotheses would be:

H0: Time using technology is the same across educational degree groups.

HA: Time using technolgy is not the same across educational degree groups.
...
The p-value in this case is `r format(welch.usetech.by.degree$p.value, scientific = FALSE)`. The value of an $F_{W}$-statistic being this large or larger happens a tiny percent of the time when the null hypothesis is true. 

#### NHST Steps 4 & 5: Interpret the probability and write a conclusion 

The results show another statistically significant difference in the mean of the usetech variable by degree group [F(`r welch.usetech.by.degree$parameter[1]`, `r welch.usetech.by.degree$parameter[2]`) = `r round(welch.usetech.by.degree$statistic, 2)`; p = `r format(welch.usetech.by.degree$p.value, scientific = FALSE)`]. Leslie notices that the $F$-statistic is a little larger and the degrees of freedom for the denominator is a smaller number. Kiara explains that the weighting is also used in the calculation of the denominator degrees of freedom [@field2013discovering]:

$$
\begin{equation}
df_{denom}=\frac{1}{\frac{3\sum_{j=1}^k\bigg(1-\frac{w_k}{\sum_{j=1}^k{w_k}}\bigg)^2}{n_k-1}}
 (\#eq:cov)
\end{equation}
$$
...
Leslie reviews the plot and notices that, while different, the distributions do not seem to be **very** different. Nancy notes that while it is true that there isn't much difference between the distributions, the area under the curves is what matters for the p-value cutoff. When the line is just slightly closer to the x-axis, this changes things quickly for the area under the curve. The thresholds for statistical significance (p < .05) for these three lines are `r qf(.95, 4, 2000)` for the 2000 degrees of freedom, `r qf(.95, 4, 25)` for the 25 degrees of freedom, and `r qf(.95, 4, 10)` for the 10 degrees of freedom. Nancy reminds Leslie that the numerator degrees of freedom has a bigger impact on the significance threshold. For example, an ANOVA with 2 degrees of freedom and the same 2000 degrees of freedom in the denominator would have a threshold for significance of `r qf(.95, 2, 2000)` instead of `r qf(.95, 4, 2000)` for the 4 and 2000 threshold.

Leslie asks if the $\omega^2$ can be used to determine effect size for Welch $F$ and Brown-Forsythe $F$. Kiara has not seen any resources that explicitly show this as an option, but the Skidmore & Thompson paper suggests it may be biased, especially when groups being compared are different sizes and the homogeneity of variances assumption fails [@skidmore2013bias]. At this point, Kiara suggests using descriptive statistics and graphs to demonstrate differences until the statistical theory catches up. 

### The Kruskal-Wallis test for failing the normality assumption

The Kruskal-Wallis test is used to compare three or more groups when the normal distribution assumption fails for ANOVA. Like several of the tests used when the the outcome is not normally distributed for a t-test, the Kruskal-Wallis test compares ranks among groups. Specifically, the observations are put in order by size and each is assigned a rank. The mean rank for each group is then computed and used to calculate the KW test statistic, $H$:

$$
\begin{equation}
H=\frac{12}{n(n+1)}\sum_{j=1}^k{n_j(\overline{r}_j-\frac{n+1}{2})^2}
 (\#eq:kwtest)
\end{equation}
$$

In this formula, $n$ is the overall sample size, $n_j$ is the sample size for group $j$ and $\overline{r}_j$ is the mean rank for group $j$. 
...
The p-value in this case is `r format(kw.usetech.by.degree$p.value, scientific = FALSE)`. The value of an $H$-statistic being this large or larger happens a tiny percent of the time when the null hypothesis is true. 

#### NHST Steps 4 & 5: Interpret the probability and write a conclusion 

The conclusion is that there is a difference in the mean rank for technology use by degree group (H(`r kw.usetech.by.degree$parameter`) = `r kw.usetech.by.degree$parameter`; p < .05). Like the ANOVA results, the KW test identifies if there is a difference somewhere among the means but does not identify which groups are different from one another. A post-hoc test like _Bonferroni_ or _Tukey_ could help. For KW, _Dunn's test_ of multiple comparisons is useful for identifying which groups are _statistically significantly_ different from which other groups.

#### Post-hoc test for Kruskal-Wallis

Dunn's test command `dunn.test()` is available in the <span style="font-family:Lucida Console, monospace;font-weight:bold">dunn.test</span> package. After installing the package, the command requires a method be selected for adjusting the p-value to account for the multiple comparisons. Nancy suggests Bonferroni since that is one of the methods commonly used with ANOVA. She shows Leslie that `dunn.test()` takes three arguments. The `x =` argument is the continuous variable, the `g = ` argument is for the groups, and the `method =` argument suggests the p-value adjustment method.
...
The new variable looks good and Leslie goes ahead with a graph:
...
gss.2018.cleaned %>%
  ...
  scale_color_manual(values = c("#78A678", "dodgerblue2","#7463AC", 
                               "deeppink", "gray80"), guide = FALSE) +
  theme_minimal() + 
  labs(x = "Degree", y = "Ranks of technology use time")

```

Leslie notices that the plot clearly demonstrates the significant differences seen in the post-hoc tests. The three college groups are very similar to one another, and there are differences among the other groups.

#### Effect size for Kruskal-Wallis

Leslie asks if there is also an effect size to report with the Kruskal-Wallis $H$? Kiara explains that $\eta^2$ works for Kruskal-Wallis, and is computed [@cohen2008explaining]:
...
For the Kruskal-Wallis test of technology use by degree, there are 936 NA values out of 2345 observations in the data frame, so $n=2345-936=1409$.

$$
\begin{equation}
\eta^2_H=\frac{142.21-5+1}{1409-5}=.098
 (\#eq:kweta2)
\end{equation}
$$

The cutoff values are the same as for the $\omega^2$:

* $\eta^2$ = .01 to $\eta^2$ < .06 is a small effect 
* $\eta^2$ = .06 to $\eta^2$ < .14 is a medium effect 
* $\eta^2 \ge$ .14 is a large effect 

In this case, consistent with the ANOVA results, the effect size for the Kruskal-Wallis test is medium. There is a medium strength relationship between degree and time using technology. Leslie writes her interpretation:

> A Kruskal-Wallis test found a statistically significant difference in technology use time across degree groups ($H$ = 142.21; p < .05). Based on post-hoc tests, those with less than a high school education had statistically significantly lower ranked times than all of the other groups (p < .05), people with a bachelor's degree, a master's degree, or a junior college degree also had significantly higher ranks than those with a high school diploma. There were no significant differences among the three college groups. There was a medium effect size for the relationship between degree and ranked values of technology use ($\eta^2 = .098$).
...
Nancy explains that this pattern of differences is consistent with an interaction. She suggests that they look at a traditional **means plot** to visualize the idea of an interaction. Although line graphs are ideally not used to display means by groups, they do aid in understanding what an interaction looks like. Nancy writes a little code so they can better understand any possible interaction:
...
Leslie reviews the means and finds that the most striking difference among the means for males and females is in junior college, when males spend a mean of 47.0% of their time on technology while females spend 70.4% of their time. Within each education group, the other means for males and females are much closer. Leslie starts the NHST process:
...
The p-values in this case were `r tidy(techuse.by.deg.sex)$p.value[1]`, `r tidy(techuse.by.deg.sex)$p.value[2]`, and `r tidy(techuse.by.deg.sex)$p.value[3]`. These are very tiny p-values, for example, the value of an $F$-statistic being as large or larger than the $F$-statistic for the DEGREE main effect happens a tiny percent of the time when the null hypothesis is true. 
...
Kiara explains that the output is showing one comparison per row. The first section is comparing groups of degree to each other. The second section is comparing males and females to each other. The third section is the interaction, comparing groups of degree+sex with each other. For example, the first row in this last section is **high school:male-< high school:male**, which compares **high school male** to **< high school male**. The numbers that follow are the difference between the means (diff = 17.8132060), the confidence interval around the difference (95% CI: 2.7275183 to 32.8988937), and the p-value for the difference between the means (p = 0.0072699). This indicates that there is a statistically significant (p < .05) difference of 17.81 between the means for these two groups *in the sample* and that the difference between the means is somewhere between 2.73 and 32.90 in the population this sample came from. 
...
Leslie reminds herself that the null hypthesis for the Shapiro-Wilk test is that the distribution is normal. By rejecting this null hypothesis with a tiny p-value, the assumption is failed. Leslie graphs the residuals to confirm. She finds that the `ggplot()` command does not work with the ANOVA object, so she converts the residuals to a new data frame first and then graphs them:
...
The residuals do not appear to be normally distributed. Instead, Leslie notices, they seem bimodal, with some large residuals at the lower end and some at the upper end of the range. This indicates that some observations are quite far below or above the group mean value for their group. 
...
agg.gss.2018 <- gss.2018.cleaned %>% 
  drop_na(USETECH) %>%
  group_by(DEGREE, SEX) %>%
  summarize(m.usetech = mean(USETECH))
```

Now that she has a new data frame with the summary data in it, she looks at the help 
...
Nancy makes some plots of the ranks to add some context to the results:
...
The plots show a the difference from one degree group to the other and between males and females. Leslie writes her interpretation of the ranked outcome ANOVA: 

> A two-way ANOVA with ranked technology time use as the outcome found statistically significant main effects of degree and sex on technology use (p < .05) and a statistically significant interaction between degree and sex (p < .05). The overall pattern of results indicates that males and females with less than a high school education use technology the least, while those with a college degree use technology the most. Males and females differ a lot in use of technology for those with a junior college degree, with females having a junior college degree having the highest use of technology of all females. 
...
5) (**A2**, **A5**) Conduct the appropriate test to compare mean time using tech across happiness groups. If the $F$-statistic is significant, use a post-hoc test to determine which means are statistically significantly different from each other. Interpret your results. 
...
7) (**A6**) Conduct a two-way ANOVA with time using technology by sex and happiness. Interpret results and check assumptions.

#### Chapter exercises: Hacker edition

Complete the coder edition and:

* Use the graphics in #3 to hypothesize which happiness groups have different tech use means. Write planned comparisons instead to test your hypotheses and conduct an appropriate planned comparison analysis instead of post-hoc tests in #5. 
...
#### Nancy's fancy code: nested and non-nested commands {#ch7nancy}

<img align = "left" src = "graphics\nancy.gif" style="PADDING-RIGHT: 30px">

sometimes it is more efficient, although not always more clear, to nest R functions inside each other. Nested functions work from the inside to the outside, so if function A is nested inside function B, function A will run first and the results of function A will be passed to function B. For example, there are nested and non-nested ways to compute the TukeyHSD post-hoc test: 
...
# use the ANOVA object in TukeyHSD
tukey.tech.by.deg <- TukeyHSD(anova.for.Tukey )
tukey.tech.by.deg
```

##### Nested

Put the ANOVA analysis inside the TukeyHSD command to do the ANOVA first and then pass its results directly to `TukeyHSD()`:
...
The nested version is less code overall, but if the ANOVA object is useful for other reasons, the non-nested option may be more efficient. Both versions give the same results, it just depends on the purpose and structure of you code for which is more clear and efficient.

#### Kiara's reproducibility resource: accessing elements of a list {#ch7kiara}

<img align = "left" src = "graphics\kiara.gif" style="PADDING-RIGHT: 30px">

Kiara wants to demonstrate how working with R code is going to be more precise and therefore easier to reproduce than copying results and doing the math by hand, even if it does seem more efficient. She demonstrates by computing the $\omega^2$ for the ANOVA by hand and with R. To do this math in R from the `aov()` object **techuse.by.deg.aov**, some fancy code is needed to access the list that is created with `summary()`. The object created by `summary()` is a list.  
...
Looking at the list in the environment pane, there are several entries in the list (Figure \@ref(fig:summ.obj)). 

```{r summ.obj, echo = FALSE, fig.cap="Screenshot of GSS Data Explorer 2018 USETECH variable."}
knitr::include_graphics("graphics/chap7-summ.object.JPG")
```

To access the parts of the list, square brackets are `[]` are used. A single element of the list requires double square brackets `[[]]` with the position of the element inside. So, for the **summ.tech.anova** object, use:
...
Great! If you need a single number from this first list element, use the single square brackets `[]` to get the number. Inside single square brackets put the position of the number you need with row first and column second. For example, to get the $F$ value from this list element, use row 1 and column 4:
...
Voila! There is the $F$ statistic all by itself. This strategy could be used to compute the $\omega^$ using code instead of retyping numbers. Remember the formula:
...
Now use R code to get the value. The degrees of freedom for degree are $k-1$ and the degrees of freedom for the residuals are $n-k$, so these can be used to get $k$ and $n$: 
...
The $\omega^2$ is a little larger, likely due to the rounding of the $F$ statistic in the by-hand version of the calculations. Kiara is satisfied that this has demonstrated the need to use R code for reproducibility. Leslie grudgingly agrees even though the code seems really long and complicated for a .002 difference. 
