...
The p-value in this case is `r format(usetech.t.by.degree$p.value, scientific = FALSE)`. The value of an $F_{BF}$-statistic being this large or larger happens about `r 100*as.numeric(format(usetech.t.by.degree$p.value, scientific = FALSE))`% of the time when the null hypothesis is true. 

#### NHST Steps 4 & 5: Interpret the probability and write a conclusion 

Leslie writes her conclusion:

> The results show a statistically significant difference in the mean of the transformed usetech variable by degree group [F(`r usetech.t.by.degree$parameter[1]`, `r usetech.t.by.degree$parameter[2]`) = `r round(usetech.t.by.degree$statistic, 2)`; p = `r format(usetech.t.by.degree$p.value, scientific = FALSE)`]. 
...
The p-value in this case is `r format(welch.usetech.by.degree$p.value, scientific = FALSE)`. The value of an $F_{W}$-statistic being this large or larger happens about `r 100*as.numeric(format(welch.usetech.by.degree$p.value, scientific = FALSE))`% of the time when the null hypothesis is true. 
...
The p-value in this case is `r format(kw.usetech.by.degree$p.value, scientific = FALSE)`. The value of an $H$-statistic being this large or larger happens about `r 100*as.numeric(format(kw.usetech.by.degree$p.value, scientific = FALSE))`% of the time when the null hypothesis is true. 

#### NHST Steps 4 & 5: Interpret the probability and write a conclusion 

The conclusion is that there is a difference in the mean rank for technology use by degree group (H(`r kw.usetech.by.degree$parameter`) = `r kw.usetech.by.degree$parameter`; p = `r format(kw.usetech.by.degree$p.value, scientific = FALSE)`). Like the ANOVA results, the KW test identifies if there is a difference somewhere among the means but does not identify which groups are different from one another. A post-hoc test like _Bonferroni_ or _Tukey_ could help. For KW, _Dunn's test_ of multiple comparisons is useful for identifying which groups are _statistically significantly_ different from which other groups.
