...
Leslie is curious about policies and programs that might address this growing problem. After a little more research she finds that states have begun to adopt a number of policies to try to combat the opioid epidemic. She finds that some of the state-level policy solutions to addressing the increasing number of opioid overdoses are [@RefWorks:2322]: 
...
Leslie is interested in how she got the County Health Roadmap data together to make Figure \@ref(fig:c4intro1), so Nancy sends her the code so she can review it later (see Box \@ref(ch4nancy)).
...
Nancy is interested in treatment programs as well as policies. She had a friend whose son became addicted to opioids after they were prescribed following a minor surgery. Her friend did some research and found that Medication-Assisted Therapies were twice as effective as other forms of treatment [@connery2015medication], but unfortunately, there were no treatment facilities with Medication-Assisted Therapies [@volkow2014medication] in the rural area where they lived. The nearest facility with these therapies was in a larger town about 40 miles away, which put an additional barrier in her way to getting effective help for her son. Having lived in a larger city most of her life, Leslie had never considered distance as a factor in why people get healthcare and treatment before. She searches for information about this and finds amfAR, the American Foundation for AIDS Research, which has an *Opioid & Health Indicators Database* [@OpioidHealthIndicatorsDatabase] with data including distance to the nearest substance abuse treatment facility that has medication assisted therapies and the distance to the nearest needle exchange program. Kiara thinks this sounds like good data to use to learn about the normal distribution. The team is satisfied that they have some meaningful data to learn more about policies and programs that address the ongoing opioid problem in the US. They are ready to start learning about distributions and inference.
...
Kiara adds that there are many different probability distributions, with each one falling into one of two categories: **discrete** or **continuous**. Discrete probability distributions show probabilities for variables that can only take certain values, which includes categorical variables and variables that must be measured in whole numbers like *number of people texting during class*. Continuous probability distributions show probabilities for values, or ranges of values, of a continuous variable that can take any value in some range. 
...
## Achievement 3: Understanding the characteristics and uses of a the normal distribution of a continuous variable {#normdistch4}
...
Kiara explains that the normal distribution is used to find the likelihood of a certain value or range of values for a continuous variable. For example, she has calculated the mean distance to the nearest substance abuse facility providing medication assisted treatment and found that it is 24.04 miles with a standard deviation of 22.66 miles. Like the probabilities from the binomial distribution are shown visually in a probability mass function graph, the normal distribution has a **probability density function** graph. Using the mean and standard deviation of 24.04 and 22.66, the probability density function graph for a variable with a mean of 24.04 and a standard deviation of 22.66 would look like:

```{r echo=FALSE, fig.cap="Probability density for a variable with a mean of 24.04 and the standard deviation of 22.66."}
#plot normal curve using ggplot
dat<-with(density(rnorm(1000000, mean=24.04, sd=22.66)),data.frame(x,y))
ggplot(data = dat, mapping = aes(x = x, y = y)) +
    geom_line()+
  theme_minimal() + 
  xlab("Miles to the nearest facility") + ylab("Probability density")
```

Leslie notices that this graph extends into negative numbers, which does not make sense for a measure of distance. She remembers back to Section \@ref(messy) and thinks that this variable may be *skewed* to the right rather than normally distributed given the large standard deviation relative to its mean. Her first thought was *leptokurtic*, but since the left side of the distribution has to stop at zero, it is more likely that it is right skewed. Kiara thinks this sounds likely and suggests that they **transform** the variable so they can continue to discuss the normal distribution. For variables that are right skewed, a few transformations that work to make the variable more normally distributed are: square root, cube root, reciprocal, and log [@manikandan2010data]. 

Nancy sends Leslie the data so she can learn how to do the transformation. To follow along, use the **opioid_dist_to_facility_2017_ch4.csv** data available at edge.sagepub.com/harris1e. Leslie opens the data and reviews it:

```{r}
# distance to substance abuse facility with medication assisted treatment
# https://opioid.amfar.org/indicator/dist_MAT
dist.mat <- read.csv("data/opioid_dist_to_facility_2017_ch4.csv")
...
Leslie starts by graphing the distance variable to confirm whether skew is the problem. Nancy lets Leslie know that the VALUE variable is the distance in miles, so this is the variable to work with.

```{r fig.cap="Miles to the nearest substance abuse treatment facility with medication-assisted therapy (2017)."}
# graph the transformations
dist.mat %>%
  ggplot(aes(x = VALUE)) +
  geom_histogram(fill = "#7463AC", color = "white") +
  theme_minimal() +
  labs(x = "Miles to nearest facility")
```



The variable is skewed indeed! Leslie tries the four transformations to see which seems more useful for making the distance variable more normally distributed.

```{r fig.cap = "Selecting a transformation to make the distance variable more normal for analysis."}
...
cuberoot <- dist.mat.cleaned %>%
  ...
  geom_histogram(fill = "#7463AC", color = "white") +
  theme_minimal() +
  labs(x = "Cube root of miles to nearest facility")

logged <- dist.mat.cleaned %>%
  ggplot(aes(x = miles.log)) +
  geom_histogram(fill = "#7463AC", color = "white") +
  theme_minimal() +
  labs(x = "Log of miles to nearest facility")

inversed <- dist.mat.cleaned %>%
  ggplot(aes(x = miles.inverse)) +
  geom_histogram(fill = "#7463AC", color = "white") +
  theme_minimal() + xlim(0,1) +
  labs(x = "Inverse of miles to nearest facility")

squareroot <- dist.mat.cleaned %>%
  ggplot(aes(x = miles.sqrt)) +
  geom_histogram(fill = "#7463AC", color = "white") +
  theme_minimal() +
  labs(x = "Square root of miles to nearest facility")

gridExtra::grid.arrange(cuberoot, logged, inversed, squareroot)
```

The cube root seems to be the best for making the distribution more normal. The inverse did not seem to work at all and made the variable appear even more skewed than it originally was. The log and square root both seem fine, but Leslie decides the cube root seems closest to normal and she chooses this transformation. Nancy goes ahead and gets the mean and standard deviation of the cube root miles so they can get back to plotting the probability distribution they were discussing before:
...
Nancy plots the probability distribution with these new summary statistics:

```{r pdfplot, echo=FALSE, fig.cap="Probability density function for a variable with a mean of 2.66 and the standard deviation of .79."}
#plot normal curve using ggplot
dat<-with(density(rnorm(1000000, mean=2.66, sd=.79)),data.frame(x,y))
ggplot(data = dat, mapping = aes(x = x, y = y)) +
    geom_line()+
  theme_minimal() + 
  xlab("Cube root of miles to the nearest facility") + ylab("Probability density")
```

That looks good for the purposes of discussing the probabilities. The go back to the original topic and Kiara explains that the area under the Figure \@ref(fig:pdfplot) curve represents 100% of observations. Using this **probability density function** graph to determine probabilities is a little different from using the **probability mass function** graph from the binomial distribution in the previous examples with categorical variables. With continuous variables the probability of any one certain number is going to be extremely low, often near zero. So, instead, probabilities are usually computed for a range of values. 

Nancy creates an example of this with the shading under the curve representing U.S. counties with the cube root of miles to a treatment facility being greater than 4 (which is 4 cubed or `r 4^3` miles to the nearest substance abuse treatment facility):

```{r c4main15, echo=FALSE, fig.cap="Probability density function for a variable with a mean of 2.66 and the standard deviation of .79."}
# plot normal curve using ggplot
dat<-with(density(rnorm(1000000, mean=2.66, sd=.79)),data.frame(x,y))
ggplot(data = dat, mapping = aes(x = x, y = y)) +
    geom_line()+
    geom_area(mapping = aes(x = ifelse(x>=4 , x, 0)), fill = "#7463AC") + ylim(0,.6) + 
  theme_minimal() +
  xlab("Cube root of miles to nearest facility") + ylab("Probability density")

```

### Finding the area under the curve

Nancy tells Leslie that the `pnorm()` command is useful for finding the actual probability value for the shaded area under the curve. In this case, `pnorm()` can be used to determine the proportion of observations that are 4 or more for the cube root of miles to the nearest facility. The `pnorm()` command takes three arguments: the value of interest, $x$, the mean ($m$), and the standard deviation ($s$).  
...
Kiara asks Leslie if 95.5% seems right. Does the shaded area in Figure \@ref(fig:c4main15) look like 95.5% of the area under the curve? Leslie shakes her head no.

Nancy lets them both know that the `pnorm()` function finds the area under the curve starting on the left up to, but not including, the x value entered, in this case 4. To get the area under the right side tail of the distribution, use the `lower.tail = FALSE` option:
...
Much better! It looks like 4.49% of observations are in the tail of this distribution and are therefore 4 or greater. Reversing the transformation, this indicates that residents of 4.49% of counties have to travel `r 4^3` miles or more to get to the the nearest substance abuse facility providing medication assisted treatment.

### Unlock achievement 3: Check your understanding

Figure \@ref(fig:c4main20) shows shading for the part of the distribution that is less than 2. Look at the graph and make your best estimate (without computing the answer) of the percentage of counties in the shaded area.

```{r c4main20, echo=FALSE, fig.cap="Probability density function for a variable with mean of 2.66 and sd or .79."}
# plot normal curve using ggplot
dat<-with(density(rnorm(100000, mean=2.66, sd=.79)),data.frame(x,y))
ggplot(data = dat, mapping = aes(x = x, y = y)) +
    geom_line()+
    geom_area(mapping = aes(x = ifelse(x<2 , x, 0)), fill = "#7463AC") + ylim(0,.6) +
  theme_minimal() +
  xlab("Cube root of miles to facility") + ylab("Probability density")
  
```

Use `pnorm()` to compute the percent of counties in the shaded area. Compare your answer with the value you estimated to see if it seems accurate. Interpret the computed value and describe how you made your estimate. 
...
These characteristics of the normal distribution can be used to describe and compare how far individual observations are from a mean value. For example, in the example above, about 68% of counties are between $2.66 - .79$ and $2.66 + .79$ for the cube root of miles to the nearest substance abuse facility. So, about 68% of counties have between `r 2.66-.79` and `r 2.66+.79` cube root miles to the nearest substance abuse facility. Transforming these values back into miles would be `r (2.66-.79)^3` to `r (2.66+.79)^3` miles. In addition, about 95% of counties would be between $2.66 - .79 \cdot 2$ and $2.66 + .79 \cdot 2$ or `r (2.66 - .79*2)^3` and `r (2.66 + .79*2)^3` mile to travel to the nearest substance abuse facility.
...
Leslie first tries using Equation \@ref(eq:zscore) to calculate $z$ for a county with residents who have to travel 50 miles to the nearest facility. In the transformed miles variable, this would be a value of `r round(50^(1/3),2)`. She finds that $z = 1.29$ (Equation \@ref(eq:zscore2)), which indicates that this county has a measure of distance to the nearest clinic that is 1.29 standard deviations above the mean.
...
Likewise, Leslie finds that a county with 10 mile to the clinic, which is a value of `r round(10^(1/3),2)` in the transformed variable, has a distance that is .65 standard deviations *below* the mean (Equation \@ref(eq:zscore3); $z = -.65$):
...
Use Equation \@ref(eq:zscore) to calculate the z-score for a county where you have to drive 15 miles to the nearest substance abuse treatment facility. Interpret your result.
...
To see how the normal distribution can use sample data to understand the population, Leslie suggests using the distance to a treatment facility variable. Kiara thinks that will work and explains that they do not have to transform the variable for this demonstration because the theory they are using works for continuous variable whether they are normally distributed or not. 

### Data management 

Remembering her data management practices, Leslie uses the `rename()` code from Section \@ref(bar-graphs) to give the distance variable a name that is easier to remember and use. She then uses summarize from Section \@ref(importing-a-comma-separated-values-(csv)-file) to remind them all of the mean and standard deviation of the variable.
...
dist.mat.cleaned %>%
  summarize(mean = mean(x = distance, na.rm = TRUE),
            sd = sd(x = distance, na.rm = TRUE),
            n = n())
```

Leslie finds the mean distance to the nearest substance abuse facility that provides at least one type of medication assisted treatment is 24.04 miles away, with a standard deviation of 22.66 miles. She also notes that there are 3,214 counties in the data set representing all (or almost all) of the counties in the U.S. This is all the U.S. counties, so it is not a sample of counties, it is the population of counties.

Kiara then asks Leslie how close she thinks we could get to the true mean distance to a facility ($m = 24.04$) if she only had enough time and money to collect data on distances from 500 counties rather than all 3,124 counties? 

### Examining a sample from a population

Leslie is unsure about whether a sample will have a mean distance anywhere near the population. Luckily Nancy has some R code to help. Before they start sampling, Kiara chimes in to remind Leslie and Nancy that the `set.seed()` command is useful for reproducibility (see Box \@ref(ch4kiara). 
...
The result is 23.48, which is close to 24.04, but not exactly the same. Leslie tries it again with a different seed value:
...
This time the value was 23.51, which is slightly higher than the first sample and a little closer to the population value of 24.04.
...
mean(x = sample.20.means$mean.distance)
```

The mean of the 20 sample means is 23.71, which is even closer to 24.04 than either of the first two samples. Leslie wants to try even more samples to see if it is even closer:
...
# find the mean of the sample means
mean(x = sample.100.means$mean.distance)
```

The mean of 100 sample means is 24.007, which is even closer to the true population mean of 24.04. 
...
# find the mean of the sample means
mean(x = sample.1000.means$mean.distance)
```

```{r fig.cap="Mean miles to a substance abuse facility in 1000 samples of 500 counties each"}
# histogram of the 1000 means
sample.1000.means %>%
  ggplot(aes(x = mean.distance)) +
  geom_histogram(fill = "#7463AC", color = "white") +
  labs(x = "Mean distance to facility from 1000 samples") +
  theme_minimal()
```

Leslie notes that the mean of the sample means of 24.07 is very close to the population mean of 24.04 and the graph of the sample means looks a lot like a normal distribution. Kiara explains that taking a lot of large samples and graphing their means results in a **sampling distribution** that looks like a normal distribution, and, more importantly, _the mean of the sample means is nearly the same as the population mean_. A **sampling distribution** is the distribution of summary statistics from repeated samples taken from a population.

### The Central Limit Theorem

Kiara explains that this phenomenon is called the _Central Limit Theorem_ and that it holds true for continuous variables that are normally distributed and those that not normally distributed. The **Central Limit Theorem** is one of the most important ideas for _inferential statistics_, or statistical approaches that infer population characteristics based on sample data. 
...
Leslie notes that the results are similar (1.01 and .91) but not identical. 
...
Both of the standard erro (se) values are close to the population standard deviation of .907, but are not exactly the same. The first sample standard error of 1.017 was a little above and the second sample standard error of .889 was a little below.

Kiara wants to summarize what they have just walked through:

* The standard deviation of the sampling distribution is `r sd(sample.1000.means$mean.distance, na.rm = T)` 
* The standard error from the first sample is `r round(sd(x = counties.500$distance)/sqrt(x = length(x = counties.500$distance)),2)` 
* The standard error from the second sample is `r round(sd(x = counties.500.2$distance)/sqrt(x = length(x = counties.500.2$distance)),2)` 
...
> The mean of any given sample (given good data collection practices) is *likely to be within two standard erors of the population mean for that variable*. 

Kiara explains that this is one of the foundational ideas of **inferential statistics**. 

### Standard deviation vs. standard error

Leslie stops Kiara to make sure she understands the difference between a sample standard deviation and a sample standard error. Kiara says the standard deviation is a measure of the **variability in the sample** while the standard error is an estimate of **how closely the sample represents the population.**  

So, if there were no way to measure every county to get the population mean for the distance to the nearest substance abuse facility in a county, Leslie could use the first sample of 500 counties to estimate that there is a good chance that the population mean distance is between $23.48-1.02$ and $23.48+1.02$ or `r 23.48-1.02` and `r 23.48+1.02`. There is an even higher probability that the population mean distance is between $23.48-1.02\cdot 2$ and $23.48+1.02\cdot 2$ or `r 23.48-2*1.02` and `r 23.48+1.02*2`. While it is unusual to have the population mean to compare sample results to, this time the team has it and can see that the population mean of 24.04 miles to the nearest substance abuse facility is represented well by the sample mean of 23.48 and its standard error of 1.02.
...
summ.500.counties <- counties.500 %>%
  summarize(smean = mean(x = distance),
            sd = sd(x = distance),
            se = sd(x = distance)/sqrt(x = length(x = distance)),
...
>The mean distance in miles to the nearest substance abuse treatment facility in this sample of 500 counties is `r round(summ.500.counties$smean, 2)`; the true or population mean distance in miles to a facility likely lies between `r round(summ.500.counties$lower.ci, 2)` - `r round(summ.500.counties$upper.ci,2)` (m = `r round(summ.500.counties$smean, 2)`; 95% CI = `r round(summ.500.counties$lower.ci, 2)` - `r round(summ.500.counties$upper.ci,2)`).

Nancy thinks now is a good time to go back to the visuals and examine this confidence interval with a histogram. She creates a histogram of the distance to a treatment facility showing the mean and the 95% confidence interval around the mean:

```{r echo = FALSE, fig.cap = "Miles to the nearest substance abues treatment facility and 95% CI from a 2017 sample of 500 counties in the US."}
counties.500 %>% 
  ggplot(aes(x = distance)) + 
  geom_histogram(fill = "white", color = "gray60") + 
  geom_vline(aes(xintercept = mean(distance)), color = "#7463AC", size = 1) +
  geom_vline(aes(xintercept = mean(distance)-1.96*sd(distance)/sqrt(length(distance))), color = "#7463AC", size = 1, 
             linetype = "dashed") +
  geom_vline(aes(xintercept = mean(distance)+1.96*sd(distance)/sqrt(length(distance))), color = "#7463AC"
             , size = 1, 
             linetype = "dashed") +
  labs(x = "Miles to the nearest treatment facility", y = "Number of counties") + 
  theme_minimal()
```
...
```{r echo = FALSE, fig.cap = "Miles to the nearest substance abuse treatment facility and 95% CI from a 2017 sample of 500 counties in the US."}
counties.500 %>% 
  ggplot(aes(x = distance)) + 
  geom_histogram(fill = "white", color = "gray60") + 
  geom_vline(aes(xintercept = mean(distance), color = "sampmean", linetype = "sampmean"), size = 1) +
  geom_vline(aes(xintercept = mean(distance)-1.96*sd(distance)/sqrt(length(distance)),
                 color = "sampci", linetype = "sampci"), size = 1) +
  geom_vline(aes(xintercept = mean(distance)+1.96*sd(distance)/sqrt(length(distance)), color = "sampci", linetype = "sampci"), size = 1) +
  geom_vline(aes(xintercept = 24.04, color = "popmean", linetype = "popmean"), size = 1) +
  labs(x = "Miles to the nearest treatment facility", y = "Number of counties") + 
  #xlim(10, 25) +
  theme_minimal() +
  scale_colour_manual(name = '',
         values = c('sampmean'='#7463AC','sampci'='#78A678','popmean'='deep pink'), labels = c('population mean', 'confidence interval\nfor sample mean', 'sample mean')) +
...
The population mean is inside the confidence interval and just above the sample mean. Leslie wonders what the mean and confidence interval look like for the second sample, so she uses Nancy's trick to calculate it:
...
Again the population mean is inside the confidence interval and above the sample mean. This makes Leslie wonder about the confidence intervals when they took 20, 100, and 1000 samples. Nancy thinks they can find these values by using `group_by()` and `summarize()`. Nancy tries the 20 samples data:
...
The confidence intervals that Leslie can see all contain the population mean of 24.04 miles to a facility in a county. Nancy thinks a graph would be easier to examine and goes back to Section \@ref(point-graph) to copy the code using `geom_errorbar()` with `geom_point()`. She creates a graph showing the population mean of 24.04 miles and the means and 95% confidence intervals for the 20 samples:

```{r fig.cap = "Means and 95% confidence interval of miles to the nearest substance abuse treatment facility and 95% CI from 20 samples of counties in the US."}
...
samp.20.stats %>%  
  ...
  geom_errorbar(aes(ymin = lower.ci, 
                    ymax = upper.ci),
                color = "#7463AC") +
  geom_hline(yintercept = 24.04, color = "deeppink") +
...
```{r fig.cap = "Means and 95% confidence interval of miles to the nearest substance abuse treatment facility and 95% CI from 100 samples of counties in the US."}
...
Leslie reviews the graph and sees a couple of confidence intervals that do not cross over the population mean. She asks Nancy if there is any way to highlight these in the graph. Nancy looks at the graph and sees the confidence intervals Leslie has noticed. For both of them, the upper boundary is lower than 24.04. Nancy thinks they can add to the summary statistics in order to highlight any confidence intervals that end above or below the population mean. She plays with the code a little:

```{r c4main50, fig.cap = "Means and 95% confidence interval of miles to the nearest substance abuse treatment facility and 95% CI from 100 samples of counties in the US."}
# get sample statistics
samp.100.stats <- samples.100 %>%
  group_by(sample_num) %>%
  summarize(means = mean(x = distance),
...
Leslie feels like she learned a lot today about statistics and about the seriousness of the opioid problem with people traveling a mean of 24.04 miles just to get to a treatment facility where they can get medication-assisted therapy and 18.08 people out of every 100,000 dying of opioid overdose in 2017. She also learned that, while there are some effective policies for reducing opioid use, not all states have adopted these policies. However, she is keeping in mind that the assumptions that they made about states being independent needs more attention before adopting the conclusions from today as facts. As she starts to learn more about how to use statistics from samples to learn about populations, Leslie thinks she will be better able to contribute to the research on all of these tricky problems. She starts to review the concepts from the day while Nancy and Kiara put together a brief summary and some tasks to work on. 
...
The American Foundation for AIDS Research also publishes the distance to needle exchange programs data. Needle exchange programs are important for preventing the spread of disease amoung intravenous drug users, including opioid users. Bring in the **opioid_dist_to_needle_exchange_2017_ch4.csv** file and examine the codebook **opioid_county_codebook_ch4.xlsx** from edge.sagepub.com/harris1e. 

5. **(A6)** Find the population mean and standard deviation of distance to needle exchange. The variable name is VALUE.
6. **(A3, A4)** View the data and pick one county, compute its z-score for distance to needle exchange. Interpret the z-score.
7. **(A6)** Take a random sample of 500 counties and find the mean, standard deviation, and standard error of distance for the sample. 
8. **(A6)** Compare the population mean for VALUE with the sample mean and sample standard error. 
9. **(A6)** Compute a 95% confidence interval around the sample mean for distance to needle exchange. Interpret the mean and confidence interval. 
...
Although the data are essentially the same, the data files require different code to import in order to be consistent. The `skip =` argument for the `read_csv()` command from the <span style="font-family:Lucida Console, monospace;font-weight:bold">tidyverse</span> package allows skipping any number of rows at the beginning of a file, which works well to bring in the 2017 data directly from the website. This will bring in the data with variable names but without labels: 
...
To put all the data sets together into one for graphing requires bringing in each one in a special way so that they are all consistent and then merging them together. Nancy shows Leslie how she did this with the four years, adding a lot of comments so Leslie can follow along.
...
colNames2014 <- names(read_csv("http://www.countyhealthrankings.org/sites/default/files/2014%20CHR%20analytic%20data.csv", 
                               n_max = 0))
# then get the data
# add the variable names using col_names
counties2014 <- read_csv("http://www.countyhealthrankings.org/sites/default/files/2014%20CHR%20analytic%20data.csv", 
                         col_names = colNames2014, skip=2)
...
colNames2017 <- names(read_csv("http://www.countyhealthrankings.org/sites/default/files/2017CHR_CSV_Analytic_Data.csv", 
                               skip=1, n_max = 0))

# then get the data
# add the variable names using col_names
counties2017 <- read_csv("http://www.countyhealthrankings.org/sites/default/files/2017CHR_CSV_Analytic_Data.csv", 
                         col_names = colNames2017, skip=2)

# 2015 & 2016 have no var names
# URL: http://www.countyhealthrankings.org/sites/default/files/2015%20CHR%20Analytic%20Data.csv
counties2015 <- read_csv("http://www.countyhealthrankings.org/sites/default/files/2015%20CHR%20Analytic%20Data.csv")

# URL: http://www.countyhealthrankings.org/sites/default/files/2016CHR_CSV_Analytic_Data_v2.csv
counties2016 <- read_csv("http://www.countyhealthrankings.org/sites/default/files/2016CHR_CSV_Analytic_Data_v2.csv")
...
Now there are four data sets with each data set having measure_138_numerator, which is the numerator for calculating the drug poisoning deaths in the county, and measure_138_denominator, which is the denominator for calculating the drug poisoning deaths in the county. Since there are four data sets, Nancy thinks a function would be helpful so that she doesn't have to compute the overdose death rate four separate times. 

The function should sum the numerator variable values to get the total number of overdose deaths across all counties, divide this by the sum of the denominator variable, which is the sum of the populations of the counties. The denominator number can then be multiplied by 100,000 to get the rate of overdose deaths per 100,000 people in a county. The formula in the function should be:

$$
\begin{equation}
od.death.rate = \frac{\sum{measure.138.numerator}}{100,000*\sum{measure.138.denominator}}
 (\#eq:death.rate)
\end{equation}
$$
Nancy writes the function:
...
Then uses the function to get the overdose rate for each year and puts all the rates in a data frame to graph:

```{r eval = FALSE}
# overdose rates vector
overdose.per.100k <- c(OverdoseFunc(counties2014),
                   OverdoseFunc(counties2015),
                   OverdoseFunc(counties2016),
                   OverdoseFunc(counties2017))
...
Finally, the small data frame can be used to make a completely reproducible (as long as the data sets are there) version of Figure \@ref(fig:c4intro1).

```{r eval = FALSE, fig.cap = "Reproducible version of drug overdose deaths per year."}
# make a graph
od.rates.year %>%
...
Try an example to see what happens when no `set.seed()` command is used with the distance data imported in Section \@ref(normdistch4)). 
