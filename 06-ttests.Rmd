...
$$
\begin{equation}
U_1=n_1{n_2}+\frac{n_2(n_2+1)}{2}-\sum{R_2}
 (\#eq:mannwhit2)
\end{equation}
$$
...
wilcox.test(nhanes.2016.cleaned$systolic ~ nhanes.2016.cleaned$sex,
            paired = FALSE)
...
At the widest gap between these two curves, the cumulative probability of systolic blood pressure for male and female are .11 apart, giving a test statistic of D = .11. The probability of getting a test statistic this large or larger is determined by examining the KS distribution. In this case, the probability of .11 difference between the two was very tiny (p < .05) if the null hypothesis were true, so the difference would be reported as statistically significant.

Leslie writes the final interpretation:
