...
* Achievement 3: Calculating the chi-squared statistic for the test of independence by hand and with R 
...
```{r ch5intro1, echo = FALSE, fig.cap="Opinions about ease of voting registration and mandatory voting from a Pew Research Center April 2017 survey (n = 1028)."}
...
While they are talking about observed and expected values, Kiara thinks it is a good time to bring up the **assumptions** of the chi-squared test. Leslie has heard about assumptions before, they are typically lists of requirements that must be met before using a statistical test. Kiara confirms that this is correct. She lists the assumptions for conducting a chi-squared test  [@mchugh2013chi]:
...
These assumptions seem confusing to Leslie and she asks if they can walk through them for the voting ease and race example. Kiara is fine with that. She lists the assumptions and they work together to explain how the data for the voting example meets each one:
...
Kiara explains that the groups are the different cells of the table, so White Non-Hispanic participants who think people should prove they want to vote is one group. In this example, there are eight groups and 80% of this would be 6.4 groups. So, seven or more of the groups should have expected values of 5 or more. None of the groups have expected values even close to 5; all are much higher. This assumption is met.

#### Assumption 3: The observations must be independent 

There are a couple of ways observations can be non-independent. One way to violate this assumption would be if the data included the same set of people before and after some intervention or treatment. Kiara explains that another way to violate this assumption is for the data to include siblings or parents and children or spouses or other people who are somehow linked to one another. Since people who are linked to each other often have similar characteristics, the statistical tests need to be able to account for this and the chi-squared test does not. Leslie thinks she gets it and the Pew data includes independent observations (not siblings or other related people and not the same people measured more than once) so this assumption is met.

Leslie asks what happens when the assumptions are not met for a statistical test. Kiara explains that often there is another test that can be used, depending on which of the assumptions is not met. For chi-squared, the assumption that is often a problem is the assumption about expected values being at least 5 in most cells. Kiara says they will look at an option for addressing this assumption violation as they walk through some examples. 
...
## Achievement 3: Calculating the chi-squared statistic for the test of independence by hand and with R {#chisqtest}

### Summing the differences between observed and expected values

The differences between observed values and expected values can be combined into an overall statistic showing how much observed and expected differ across all the categories [@kim2017statistical]. However, since some expected values are higher than observed values, some are lower, and the expected and observed both have the same total when summed, just combining the differences will result in a zero, like this:
...
## Achievement 5: Using Null Hypothesis Significance Testing (NHST) to organize statistical testing {#nhst}

Kiara mentions to Leslie that one process to organize statistical tests like _chi-squared_ is _null hypothesis significance testing_ or NHST. Leslie asks if this is the same as the chi-square goodness-of-fit test? Kiara says no and Leslie decides she will look that up later (see Box \@ref(ch5leslie2)). Kiara writes out the steps of NHST:
...
Another effect size useful when both variables are binary is the **odds ratio** [@kim2017statistical]. The odds ratio measures the odds of some event or outcome occurring given a particular exposure compared to odds of it happening without that exposure. In this case, the **exposure** would be home ownership and the **outcome** would be opinion on ease of voting. So, the odds ratio would measure the odds of thinking people should prove they want to vote given owning a home, compared to the odds of thinking people should prove they want to vote given not owning a home.
...
Before they are done for the day, Leslie also wants to discuss the assumptions of chi-squared and what to do when each one is violated. Kiara is fine with this although she thinks the expected cell size being less than 5 in a lot of cells is going to be the most likely assumption to cause problems. These topics do not go together very well, but they go ahead anyway. Leslie lists the assumptions again and they discuss each one:

### Violating the assumption that the variables must be nominal or ordinal (usually nominal) 

Kiara says this one is easy! Use a different statistical test. Chi-squared is only appropriate for categorical variables. 

### Violating the assumption of expected values of 5 or higher in at least 80% of groups 

The sampling distribution for the chi-squared statistic approximates the actual chi-squared distribution but does not capture it completely accurately. When a sample is large, the approximation is better and using the chi-squared distribution to determine statistical significance works well. However, for very small samples, the approximation is not great, so a different method of computing the p-value is better. The methods most commonly used is the **Fisher's Exact Test**. Essentially, the Fisher's Exact Test computes a different p-value for the $\chi^2$ statistic [@kim2017statistical]. In R, the Fisher's Exact Test can be conducted with the `fisher.test()` function. The process of NHST and the interpretation of the results are the same as for the $\chi^2$, just with a different method of computing the p-value to reject or keep the null hypothesis. 

### Violating the independent observations assumption

Leslie then asks what happens if the observation of independent assumptions is violated. Kiara looks at her with no expression and says "It depends," and then starts laughing. Seriously though, there are a few ways observations can be non-independent, Kiara explains. If the data were collected in order to examine non-independent observations, for example, some characteristic of the same set of people before and after an intervention, or whether two siblings share some characteristic or opinion, **McNemar's test** is a good alternative to the $\chi^2$ test, but only when both of the variables have two categories [@mccrum2008correct]. McNemar's test is available in R with the `mcnemar.test()` function. If there are three or more dependent groups with a binary outcome **Cochran's Q-test** is an option [@mccrum2008correct] and is available in R in the <span style="font-family:Lucida Console,  monospace;font-weight:bold">nonpar</span> package. If the data were collected in a sloppy way and this resulted in a few people being surveyed twice for example, it is probably best to clean the data as much as possible and stick to using descriptive statistics and graphs.  

Leslie is finding all of these different tests to be a little overwhelming. How will she ever remember which test to use! Kiara reassures her that *most* of what she will need to do as a data scientist will be among the descriptive statistics and inferential tests they have already covered and plan to talk about in the upcoming weeks. She also explains that almost all of the inferential tests she is going to use can follow the same NHST process she used with the chi-squared test. The things that usually differ from test-to-test are the test statistic and the underlying distribution used to get the p-value. Leslie feels a little better but is still nervous. Nancy lets her know that the next set of tests they will learn involve beer. That sounds less difficult and Leslie relaxes a little. 
...
#### Achievement 8 recap: Understanding the options for failed chi-squared assumptions

When chi-squared assumptions fail there are options including recoding data to combine groups, choosing a different statistical test, or using descriptive statistics and graphs without a chi-squared analysis. Which options are most useful depends on which assumption(s) failed and how they failed.
...
7) (**A8**) Check the assumptions and report whether they were met
...
8) (**A7**) Write a brief discussion of whether the the assumptions for chi-squared were met in this analysis
8) Add a prolog and comments to your code
...
```{r crosstabdoc, echo = FALSE, fig.cap="CrossTable help documentation."}
...
#### Leslie's stats stuff: the chi-squared goodness-of-fit test {#ch5leslie2}

Leslie reads some more about the chi-squared goodness-of-fit test, which is different from the chi-squared test they have been working on. The one used so far tests whether there is a relationship between two categorical variables. There is another chi-squared test for comparing the values across a single categorical variable to values from a hypothesized or population variable. Leslie reads this about three times before she turns to Kiara for help. Kiara looks it over and remembers using this before when trying to determine if her sample was a good representation of the population. She thinks Leslie might benefit from a concrete example. Kiara explains that they could compare the observed values of the race-ethnicity variable to what they would expect to see in each category if race-ethnicity was the same as in the US. This one variable chi-squared is called the goodness-of-fit chi-squared test. The test statistic is still $\chi^2$ and the chi-squared distribution is still used in the same way. The test statistic itself is computed the same way, although this time the expected values are the hypothesized or population values. Leslie and Kiara work through an example using NHST to organize and using the race ethnicity variable from the Pew data:

**Step 1: Write the null and alternate hypotheses**

H0: There is no difference between the proportions of people in each race-ethnicity category in the sample and the proportions in the population.

HA: There is a difference between the proportions of people in each race-ethnicity category in the sample and the proportions in the population.

**Step 2: Compute the test statistic**

Kiara looks at the Wikipedia page for US demographics. She finds the distribution by 2000 census and matches it to the categories in the Pew data. The US population is: 
...
To compare the distribution of race in the sample to this population distribution, they will need a vector of the frequencies in the sample to compare to these percents. The `count()` function will count the number of observations in each category: 
...
Now that they have the frequency vector `race.eth$n`, they can use `chisq.test()` by entering the frequency vector and then entering the population proportions for each race-ethnicity in the same order as the values in the vector. The two arguments are `x =` for the observed vector and `p = ` for the population vector.
...
The probability that the null hypothesis is true is `r round(race.gof$p.value, 3)`.

**Step 4: If the probability that the null is true is very small, usually less than 5%, reject the null hypothesis** 

A $\chi^2$ goodness-of-fit test comparing the frequencies of race-ethnicity in the Pew data to the proportions in each race-ethnicity in the population found a statistically significant difference between the observed and the population values ($chi^2$(`r race.gof$df`) = `r round(race.gof$statistic, 2)`; p = `r round(race.gof$p.value, 3)`). 

**Step 5: If the probability that the null is true is not small, usually 5% or greater, retain the null hypothesis**

Not applicable.

Leslie thinks they should take a look at a graph to better understand these results. Nancy jumps in to help with some quick coding:

```{r}
# add percents of race-ethnicity from pew data
...
Leslie doesn't think the groups look all that different from each other. There are fewer White Non-Hispanic and more Hispanic and Other in the sample, but none of the differences is huge. Nancy suggests she check out the standardized residuals from the `race.gof` object. Leslie did not even know this was a thing! Nancy explains that she can find what is part of objects resulting from functions like `chisq.test()` in the help documentation [@chisqtestfunction]. In this case, the `race.gof` object resulting from the goodness-of-fit test includes `stdres`, so Leslie gives it a try:
...
The standardized residuals are in the same order as the categories of race-ethnicity, so it appears that the proportion of White Non-Hispanic in the sample was much lower than expected (standardized residual = -3.22) while the proportion of Hispanic participants in the sample was much higher than expected (standardized residual = 2.35). Leslie is satisfied with this and writes her full interpretation: 

> A $\chi^2$ goodness-of-fit test comparing the proportion of participants in race-ethnicity groups in the Pew data to the proportions in each race-ethnicity group in the US population found a statistically significant difference between the observed sample and the population values ($/chi^2$(`r race.gof$df`) = `r round(race.gof$statistic, 2)`; p = `r round(race.gof$p.value, 3)`). Standardized residuals indicated there were fewer White Non-Hispanic participants and more Hispanic participants in the sample than would be expected based on the population proportions.
